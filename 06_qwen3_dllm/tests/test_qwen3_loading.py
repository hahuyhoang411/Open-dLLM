...
def test_forward_matches_hf():
    ...
    assert hf_logits.shape == our_logits.shape, (
        f"Shape mismatch: {hf_logits.shape} vs {our_logits.shape}"
    )
...
def test_embedding_values_match():
    ...
    # Embedding weights
    assert torch.equal(hf_model.model.embed_tokens.weight, our_model.token_emb.weight), \
        "Embedding weights don't match"

    # Layer 0 Q projection
    assert torch.equal(
        hf_model.model.layers[0].self_attn.q_proj.weight,
        our_model.blocks[0].attn.c_q.weight
    ), "Layer 0 Q proj weights don't match"

    # Last layer MLP gate
    assert torch.equal(
        hf_model.model.layers[27].mlp.gate_proj.weight,
        our_model.blocks[27].mlp.gate_proj.weight
    ), "Layer 27 gate_proj weights don't match"

    # Final norm
    assert torch.equal(
        hf_model.model.norm.weight,
        our_model.final_norm.weight
    ), "Final norm weights don't match"

    # QK norms
    assert torch.equal(
        hf_model.model.layers[0].self_attn.q_norm.weight,
        our_model.blocks[0].attn.q_norm.weight
