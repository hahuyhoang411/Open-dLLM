...
from phase6.schedule import compute_elbo_weight, apply_noise, sample_timesteps
from phase6.data import _compute_positions, _sample_t_per_doc, _apply_noise_per_doc
from phase6.optim import MuonClip, _MaxLogitsTracker, build_param_groups, create_optimizer
...
class TestQKNormRoPEOrdering:
    ...
    def test_model_uses_norm_before_rope(self):
        ...
        assert torch.allclose(captured['q'], q_expected, atol=1e-5), \
            "Model Q doesn't match norm-then-rope path"
        assert torch.allclose(captured['k'], k_expected, atol=1e-5), \
            "Model K doesn't match norm-then-rope path"
...
class TestGQARepeat:
    ...
    def test_sdpa_output_matches_manual_gqa(self):
        ...
        assert torch.allclose(out_sdpa[:, 0], out_manual, atol=1e-5), \
            "SDPA head 0 doesn't match manual GQA attention"
...
class TestRoPEBase1M:
    ...
    def test_base1m_weaker_high_frequency(self):
        ...
        assert ratio > 50, \
            f"base=10K last inv_freq should be >50x larger than base=1M, got ratio {ratio:.1f}"
...
class TestELBOWeightBoundaries:
    ...
    def test_elbo_weight_below_t_min_clamped(self):
        ...
        w = compute_elbo_weight(t, t_min=0.1)
        assert torch.allclose(w, torch.tensor([[10.0, 10.0, 10.0]])), \
            f"Expected all 10.0 for t < t_min, got {w}"
    ...
    def test_elbo_weight_per_token_in_compute_loss(self):
        ...
        assert ratio == pytest.approx(5.0, rel=1e-3), \
            f"10x weight vs 2x weight should give 5x ratio, got {ratio:.4f}"
...
class TestLossNormalization:
    ...
    def test_loss_normalizes_by_all_real_tokens(self):
        ...
        assert 0.3 < ratio < 0.7, \
            f"50% mask should give ~50% loss of full mask (same denominator). Got ratio {ratio:.3f}"
    ...
    def test_padding_excluded_from_denominator(self):
        ...
        assert 0.5 < ratio < 2.0, \
            f"Padded loss / real loss ratio {ratio:.3f} out of expected range"
...
class TestChunkBoundary:
    ...
    def test_chunked_matches_full_computation(self):
        ...
        loss_chunked = compute_loss(
            h.detach().requires_grad_(True), targets, mask, elbo_w, w_lm, _FakeCfg()
        )
...
class TestDynamicRemasking:
    ...
    def test_commit_all_above_tau(self):
        ...
        assert commit.sum().item() == 6, \
            f"Expected 6 commits (all above tau), got {commit.sum().item()}"
...
class TestPerDocTRange:
    ...
    def test_no_t_below_t_min_1000_samples(self):
        ...
        assert all_min >= 0.1 - 1e-7, \
            f"Found t={all_min} below t_min=0.1 in 1000 samples"
...
class TestNewtonSchulzOrthogonal:
    ...
    def test_ns5_gram_matrix_near_identity(self):
        ...
        assert ratio < 0.3, \
            f"Off-diagonal/diagonal ratio={ratio:.4f}, not near-orthogonal"
...
class TestEmptyMask:
    ...
    def test_all_unmasked_zero_loss(self):
        ...
        assert loss.item() == pytest.approx(0.0, abs=1e-6), \
            f"All-unmasked loss should be 0, got {loss.item()}"
...
class TestStaircaseMaskDocBoundaries:
    ...
    def test_same_doc_attention_allowed(self):
        ...
        assert mask[0, 0, 4, 16].item() == 0.0, \
            "x_t[block1] should attend x_0[block0] within same doc"
...
class TestGumbelNoise:
    ...
    def test_gumbel_preserves_relative_order_roughly(self):
        ...
        assert count_token0 > 70, \
            f"Token 0 (logit=10) should be selected >70% of the time, got {count_token0}%"
...
class TestResidualInit:
    ...
    def test_residual_projections_smaller_std(self):
        ...
        for block in model.blocks:
            ...
            assert c_proj_std < 0.02, \
                f"c_proj std {c_proj_std:.5f} should be < 0.02 (scaled init)"
            assert down_std < 0.02, \
                f"down_proj std {down_std:.5f} should be < 0.02 (scaled init)"
    ...
    def test_gated_query_zero_init(self):
        ...
        for block in model.blocks:
            ...
            assert torch.all(gate_weight == 0), \
                "w_gate should be zero-initialized for identity behavior at init"
...
class TestStaticRemasking:
    ...
    def test_exact_count(self):
        ...
        for k in [1, 2, 3, 5]:
            ...
            assert commit.sum().item() == k, \
                f"Static remasking: expected {k} commits, got {commit.sum().item()}"
    ...
    def test_schedule_sums_correctly(self):
        ...
        for blk in [4, 7, 8, 10, 16]:
            for steps in [1, 2, 3, 4, 5, 8]:
                ...
                assert schedule.sum().item() == blk, \
                    f"blk={blk}, steps={steps}: sum={schedule.sum().item()}"
