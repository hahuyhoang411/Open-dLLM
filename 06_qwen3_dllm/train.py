...
from phase6.config import _check_flex, _check_trackio, from_cli, setup_device, setup_features
...
def _generate_sample(model, cfg, prompt='The capital of France is', max_new_tokens=64, temperature=0.8):
  ...
  try:
    ...
    with disable_fp8(model) if cfg.use_fp8 else contextlib.nullcontext():
      token_ids = generate(model, prompt_ids, cfg, max_new_tokens=max_new_tokens, temperature=temperature)
...
@torch.no_grad()
def estimate_loss(model, get_batch_fn, cfg, *, eval_iters=None, splits=('train', 'val'), reset_val_fn=None):
  ...
  for split in splits:
    ...
    for k in range(n_iters):
      ...
      with torch.amp.autocast('cuda', dtype=torch.bfloat16, enabled=cfg.use_amp):
        ...
        loss_fn = compute_loss_flce if cfg.use_flce else compute_loss
        loss = loss_fn(hidden, targets, mask, elbo_w, model.lm_head.weight, cfg)
      losses[k] = loss.item()
...
if __name__ == '__main__':
  ...
  if cfg.master_process:
    ...
    lines = [
      '=' * 60,
      'phase6_dllm -- Qwen3 block diffusion LM (Phase 6)',
      '=' * 60,
      ...
      f'  flags          = amp={cfg.use_amp} liger={cfg.use_liger} flce={cfg.use_flce} flex={cfg.use_flex} cart={cfg.use_cart}',
      ...
      '  noise_schedule = linear (ELBO 1/t), doc_packing + per-doc t',
      '=' * 60,
    ]
    print('\n'.join(lines))
  ...
  if pathlib.Path(weights_path).exists() and not cfg.train:
    ...
    if cfg.ddp:
      model = DDP(
        model,
        device_ids=[int(cfg.device.split(':')[-1])],
    ...
    if cfg.master_process and trackio_ok:
      tkw = {
        'project': 'smoldlm-phase6',
        'config': {
          'arch': f'{cfg.n_layer}L/{cfg.n_embd}d/{cfg.n_head}h',
          'seq_len': cfg.seq_len,
          'batch_size': effective_batch,
          'max_iters': cfg.max_iters,
          'hf_model': cfg.hf_model_name,
    ...
    for step in range(start_step, cfg.max_iters):
      ...
      for micro_step in range(cfg.grad_accum_steps):
        ...
        with ctx:
          ...
          with torch.amp.autocast('cuda', dtype=torch.bfloat16, enabled=cfg.use_amp):
            hidden, _ = model(x_input, targets, attn_mask=attn_mask, positions=positions)
            loss_fn = compute_loss_flce if cfg.use_flce else compute_loss
            loss = loss_fn(hidden, targets, mask, elbo_w, raw_model.lm_head.weight, cfg)
      ...
      avg_tps = (
        int(((ss - _WARMUP_STEPS + 1) * effective_batch * cfg.seq_len) / max(total_training_time, 1e-9))
