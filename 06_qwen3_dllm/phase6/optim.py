...
# gram-newton-schulz: drop-in replacement for newton_schulz when cfg.use_gram_ns=True
_gram_ns_instance = None


def enable_gram_ns():
    global _newton_schulz_compiled, _gram_ns_instance
    from gram_newton_schulz import POLAR_EXPRESS_COEFFICIENTS, GramNewtonSchulz

    _gram_ns_instance = GramNewtonSchulz(
        ns_coefficients=POLAR_EXPRESS_COEFFICIENTS,
        gram_newton_schulz_reset_iterations=[2],
        ns_use_kernels=False,  # faster than True at D=1024
    )

    @torch.no_grad()
    def _gram_ns_fn(G, steps=5, eps=1e-7):
        return _gram_ns_instance(G.contiguous())

    _newton_schulz_compiled = _gram_ns_fn
...
def build_param_groups(model, cfg: Config):
    ...
    if qk_muon:
        groups.append(dict(
            params=qk_muon, lr=cfg.muon_lr, momentum=0.95, weight_decay=0.1,
            apply_muon=True, is_qk=True, qk_clip_tau=100.0,
        ))

    if other_muon:
        groups.append(dict(
            params=other_muon, lr=cfg.muon_lr, momentum=0.95, weight_decay=0.1,
            apply_muon=True, is_qk=False,
        ))

    if adamw:
        groups.append(dict(
            params=adamw, lr=cfg.adamw_lr, betas=(0.9, 0.95), eps=1e-8,
...
def _build_adamw_groups(model, cfg: Config):
    ...
    groups = []
    if decay:
        groups.append(dict(params=decay, lr=cfg.adamw_lr, betas=(0.9, 0.95),
                           eps=1e-8, weight_decay=0.1))
    if no_decay:
        groups.append(dict(params=no_decay, lr=cfg.adamw_lr, betas=(0.9, 0.95),
