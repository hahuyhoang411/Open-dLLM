...
All functions take explicit parameters or a Config object — no module-level globals.
"""
...
def sample_timesteps(batch_size, num_blocks, block_size, t_min=0.1):
...
def apply_noise(targets, t, mask_token_id, pad_token_id=None, block_size=8):
  """LINEAR schedule: mask_prob = t. Apply noise mask to targets.

  Per-block min-1-masked guarantee (Stable-DiffCoder): every block of
  block_size tokens has at least one masked real token.
  """
  B, L = targets.shape
  mask_prob = t
  ...
  # Per-block min-1-masked guarantee (vectorized for no-padding case)
  num_blocks = L // block_size
  mask_blocks = noise_mask.view(B, num_blocks, block_size)
  zero_blocks = mask_blocks.sum(dim=2) == 0
  if zero_blocks.any():
    if pad_token_id is not None:
      # Pad-aware: fall back to scalar loop since padding makes full
      # vectorization awkward and doc-packed sequences rarely have padding
      ...
      # Vectorized: pick a random offset within each zero block, scatter True
      rand_off = torch.randint(0, block_size, zero_blocks.shape, device=targets.device)
      block_starts = torch.arange(num_blocks, device=targets.device).unsqueeze(0).expand(B, -1)
      abs_pos = block_starts * block_size + rand_off  # (B, num_blocks)
      batch_idx, blk_idx = zero_blocks.nonzero(as_tuple=True)
      noise_mask[batch_idx, abs_pos[batch_idx, blk_idx]] = True
...
def compute_elbo_weight(t, t_min=0.1):
  """LINEAR schedule ELBO weight: 1/t (mask_prob = t, so 1/mask_prob = 1/t)."""
  return 1.0 / t.clamp(min=t_min)


def get_lr_factor(step, warmup_iters, decay_start, max_iters):
...
def compute_cart_weights(mask, padding, p=0.1):
  """CART context-adaptive ELBO weights (optional, off by default).

  More context around a masked token -> higher weight.
  Matches Dream 7B exactly: geometric kernel convolved over unmasked positions.
  """
  B, L = mask.shape
  ...
  cart_scores = F.conv1d(ctx, kernel).squeeze(1)
