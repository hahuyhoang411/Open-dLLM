...
# ============================================================================
# QK-Clip frequency control — compute norms every N forward passes, not every one.
# Cuts ~2.1 GB temporaries per layer on 9 out of 10 steps.
# ============================================================================
_qk_clip_counter = 0
_QK_CLIP_EVERY = 10


def _should_compute_qk_norms():
  global _qk_clip_counter
  _qk_clip_counter += 1
  return _qk_clip_counter % _QK_CLIP_EVERY == 1


# Import _MaxLogitsTracker at module level to avoid graph breaks from
# in-forward imports. Falls back to None if optim module not available.
try:
...
def apply_rotary_emb(q, k, cos, sin):
  # When the sequence is doubled ([x_t || x_0]) but cos/sin cover only half,
  # tile along the position dimension so both halves share the same positions.
  T = q.size(1)
  if cos.size(1) * 2 == T:
    cos = cos.repeat(1, 2, 1, 1)
    sin = sin.repeat(1, 2, 1, 1)
...
def build_staircase_mask(seq_len, blk_size, doc_ids=None):
  ...
  if doc_ids is not None:
    ...
    _zero = torch.zeros(1, dtype=torch.bfloat16, device=_device)
    _neginf = torch.tensor(float('-inf'), dtype=torch.bfloat16, device=_device)
    return torch.where(allow, _zero, _neginf).unsqueeze(1)  # (B, 1, 2L, 2L)

  return torch.where(allow, torch.zeros(1, dtype=torch.bfloat16), torch.tensor(float('-inf'), dtype=torch.bfloat16))
...
class MultiHeadAttention(nn.Module):
  def __init__(self, cfg: Config):
    ...
    # Dispatch mode: set once at init (or before compile) to avoid
    # runtime _check_flex() / isinstance() graph breaks in forward().
    if cfg.use_flex and _check_flex():
      self._dispatch = 'flex'
    else:
      self._dispatch = 'sdpa_causal'

  def set_dispatch_mode(self, mode: str):
    assert mode in ('flex', 'sdpa_mask', 'sdpa_causal')
    self._dispatch = mode
  ...
  def update_cache(self, k, v):
    if self.kv_cache is None:
      ...
      self.kv_cache = (torch.cat([old_k, k], dim=2), torch.cat([old_v, v], dim=2))
  ...
  def forward(self, x, cos, sin, attn_mask=None, positions=None):
    ...
    if positions is not None:
      ...
      k = _apply_rotary_emb(
        k, cos_pos[:, :, :1, :].expand(-1, -1, n_kv_head, -1), sin_pos[:, :, :1, :].expand(-1, -1, n_kv_head, -1)
      )
    else:
      # cos.size(1) == T: inference or training with pre-doubled buffers (cos_2x/sin_2x)
    ...
    # Uses module-level _MaxLogitsTracker (no in-forward import → no graph breaks).
    # Gated by _should_compute_qk_norms() — runs every 10th forward, not every one.
    # bf16 norms (no .float() cast) — sufficient precision for upper-bound tracking.
    if self.training and _MaxLogitsTracker is not None and _should_compute_qk_norms():
      with torch.no_grad():
        logit_bound = q.norm(p=2, dim=-1).max() * k.norm(p=2, dim=-1).max() / math.sqrt(cfg.head_dim)
