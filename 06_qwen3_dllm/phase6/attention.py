...
def _apply_rotary_emb_manual(x, cos, sin):
    d = x.shape[3] // 2
    x1, x2 = x[..., :d], x[..., d:]
    y1 = x1 * cos - x2 * sin
    y2 = x1 * sin + x2 * cos
...
# flash-attn RoPE: loaded lazily on first use when cfg.use_fa_rope=True
_fa_rope_fn = None


def _apply_rotary_emb(x, cos, sin):
    if _fa_rope_fn is not None and cos.shape[0] == 1 and cos.dim() == 4 and cos.shape[2] == 1:
        # cos: (1, T, 1, HD//2) -> (T, HD//2) for flash-attn
        return _fa_rope_fn(x, cos[0, :, 0, :], sin[0, :, 0, :], interleaved=False)
    return _apply_rotary_emb_manual(x, cos, sin)
