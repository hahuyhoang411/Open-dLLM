...
def _ddp_verify(raw_model, step, loss_val, grad_norm_val):
    ...
    info = torch.tensor([param_sum, loss_val, grad_norm_val],
    ...
    for name, p in raw_model.named_parameters():
        if p.grad is not None:
            ...
            if gnorm == 0:
                n_zero += 1
            if n_grad <= 3:
                print(f'  [GRAD] {name}: grad.norm={gnorm:.6e} '
                      f'dtype={p.grad.dtype} shape={list(p.grad.shape)}')
        else:
            n_none += 1
            if n_none <= 3:
                print(f'  [GRAD] {name}: grad=None')
    manual_norm = math.sqrt(manual_sq)
    print(f'  [GRAD] summary: {n_grad} with grad, {n_none} None, '
          f'{n_zero} zero | manual_norm={manual_norm:.6e}')
    print(f'  [GRAD] loss: requires_grad={loss.requires_grad}, '
    ...
    for split in ('train', 'val'):
        ...
        for k in range(config.eval_iters):
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16, enabled=config.use_amp):
                hidden, _ = model(x_input, targets,
                                  attn_mask=attn_mask, positions=positions)
                loss = compute_loss(hidden, targets, mask, elbo_w,
                                    model.lm_head.weight, use_liger=config.use_liger)
            losses[k] = loss.item()
        ...
        for step in range(start_step, config.max_iters):
            ...
            if step % config.eval_interval == 0 or step == config.max_iters - 1:
                ...
                if step == 0 and losses.get('train', 0) > 25:
                    raise RuntimeError(
                        f"Step-0 train loss {losses['train']:.2f} >> expected ~19-20. "
                        "Check loss normalization and ELBO weighting."
                    )
                if config.master_process:
            ...
            for micro_step in range(config.grad_accum_steps):
                ...
                with ctx:
                    ...
                    if config.debug and step == start_step and config.master_process:
                        print(f'  [PRE-BW] loss={loss.item():.4f} requires_grad={loss.requires_grad} '
                              f'grad_fn={type(loss.grad_fn).__name__ if loss.grad_fn else None}')
                        print(f'  [PRE-BW] hidden: requires_grad={hidden.requires_grad} '
            ...
            avg_tps = (int((steady_steps * effective_batch * config.seq_len)
