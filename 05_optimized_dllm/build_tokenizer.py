...
def verify(tokenizer):
    ...
    assert len(mask_enc.ids) == 1, f'<|mask|> should encode to 1 token, got {mask_enc.ids}'
    assert mask_enc.ids[0] == 0, f'<|mask|> should encode to [0], got {mask_enc.ids}'
    print('Special token encoding: <|mask|> -> [0] -> OK')
...
def main():
    ...
    if pathlib.Path(TOKENIZER_PATH).exists() and not args.force:
