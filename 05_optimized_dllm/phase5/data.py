...
Supports two modes:
1. Streaming: HuggingFaceFW/finepdfs_50BT-dclm_30BT-fineweb_edu_20BT-shuffled -> tokenize on-the-fly -> pack (default, no --data-dir)
2. Pre-tokenized: HoangHa/100BT-dLLM-pretokenized (or any HF dataset) -> batch random access -> pack (--data-dir=<repo-id>)
"""
    ...
    if _train_loader is None:
      if config.data_dir:
        ...
        if config.master_process:
          print(f'[data] Pre-tokenized: {_train_loader._n:,} docs from {config.data_dir}')
      else:
